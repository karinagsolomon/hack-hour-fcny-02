...
const maxDepth = (arr: NestedArray): number => {
  let maxChildDepth = 0; // initializing maxChildDepth

  for (const element of arr) { // looping thru random array
    if (Array.isArray(element)) { // checking if element itself is an array
      const depth = maxDepth(element);
